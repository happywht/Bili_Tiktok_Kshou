"""
抖音服务 - 基于 MediaCrawler 浏览器自动化实现

通过 MediaCrawler 桥接脚本，利用 Playwright 浏览器自动化完成抖音搜索。
...
class DouyinService(PlatformService):
    ...
    def __init__(self, cookies: str = None, ttwid: str = None, headless: bool = True):
        """
        初始化抖音服务

        Args:
            cookies: Cookie 字符串（桥接脚本通过浏览器自动管理）
            ttwid: ttwid 参数（已废弃，保留兼容）
            headless: 是否无头模式
        """
    ...
    def _resolve_bridge_path() -> str:
        """获取桥接脚本路径"""
        # api/services/douyin_service.py → tools/mediacrawler_bridge.py
    ...
    def search(
        ...
        **kwargs
        ...
        timeout = kwargs.get('timeout', 150)  # 比桥接脚本自身超时多 30s 缓冲
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Python 解释器不存在: {sys.executable}"
            )
    ...
    def _parse_result(self, result: subprocess.CompletedProcess) -> List[ContentItem]:
        ...
        if not data:
            ...
            raise RuntimeError("抖音搜索失败: 桥接脚本无有效输出")
        ...
        items = data.get('data', [])
        logger.info(f"[DouyinService] 搜索完成，获取 {len(items)} 条结果")

        return [
            self._to_content_item(item)
            for item in items
        ]
    ...
    def get_detail(self, content_id: str) -> Dict[str, Any]:
        """获取内容详情（暂不实现，返回基本信息）"""
        return {"id": content_id, "platform": "douyin", "detail": "暂不支持详情获取"}
    ...
    def check_environment() -> Dict[str, Any]:
        """检查运行环境是否就绪"""
        bridge_path = DouyinService._resolve_bridge_path()
        ...
        try:
            ...
            data = DouyinService._safe_json_parse(result.stdout)
            if data:
